...
export function createPracticeUI({ containerId, skill, defaultPart, loadData }) {
  ...
  return {
    ...
    render() {
      ...
      container.innerHTML = `<div class="quiz-container card"><div class="quiz-header"><span class="badge badge-accent">${title} Part ${part}</span><span>Mục ${index + 1}/${items.length} · ${questions.length} câu</span></div>
        ${questions[0] ? renderQuestionContext(questions[0], checked) : ''}
        ${checked ? '' : questions.map((q, qIndex) => `<section class="subquestion-card"><p class="question-text"><strong>Câu ${qIndex + 1}:</strong> ${escape(q.q)}</p>
          <div class="options-grid">${q.options.map((option, optionIndex) => `<button class="option-btn ${session.getAnswer(q.id) === optionIndex ? 'selected' : ''}" data-question="${qIndex}" data-option="${optionIndex}" aria-pressed="${session.getAnswer(q.id) === optionIndex}"><span class="option-label">${String.fromCharCode(65 + optionIndex)}</span><span>${escape(option)}</span></button>`).join('')}</div></section>`).join('')}
